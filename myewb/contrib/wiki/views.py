...
def edit_article(request, slug,
    ...
    if request.method == 'POST':
        ...
        lock = cache.get(slug, None)
        if lock is None:
            lock = ArticleEditLock(slug, request)
...
@login_required
def view_changeset(request, slug, revision,
                   group_slug=None, bridge=None,
                   article_qs=ALL_ARTICLES,
    ...
    if request.method == "GET":
        article_args = {'article__slug': slug}
        if group_slug is not None:
            try:
                group = bridge.get_group(group_slug)
            except ObjectDoesNotExist:
...
@login_required
def revert_to_revision(request, slug,
                       group_slug=None, bridge=None,
                       article_qs=ALL_ARTICLES,
                       extra_context=None,
                       is_member=None,
...
@login_required
def observe_article(request, slug,
                    group_slug=None, bridge=None,
                    article_qs=ALL_ARTICLES,
                    template_name='recentchanges.html',
                    template_dir='wiki',
...
@login_required
def stop_observing_article(request, slug,
                           group_slug=None, bridge=None,
                           article_qs=ALL_ARTICLES,
                           template_name='recentchanges.html',
                           template_dir='wiki',
...
def article_history_feed(request, feedtype, slug,
                         group_slug=None, bridge=None,
                         article_qs=ALL_ARTICLES, changes_qs=ALL_CHANGES,
                         extra_context=None,
                         is_member=None,
    ...
    try:
        feedgen = ArticleHistoryFeed(slug, request,
                                     group_slug, bridge,
                                     article_qs, changes_qs,
                                     extra_context,
                                     *args, **kw).get_feed(slug)
